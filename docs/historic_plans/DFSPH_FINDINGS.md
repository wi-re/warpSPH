...
### 1.18 A composed near-wall Jacobi needs the wall row to depend on the current iterate, not a frozen snapshot, to contract — confirmed on a second, independent solve (Part 56)

Part 41 diagnosed this once, for `omniIncompressible`'s density Jacobi: freeze
`kind==1` pressure at `p ≡ 0` (or any pre-solve snapshot) and the near-wall
iteration matrix `D` carries a wall term the operator `A p` does not — a
classic non-contraction. The fix, `wallPressureExtrapolation`, recomputes the
wall row from the *current* fluid iterate every sweep (a Robin closure), and
that was believed to be a property of that one solve.

Part 56 found the identical mechanism, independently, in a completely
different solve: the VD+PS shift's own constant-density `solveIncompressible`
(`modules/incompressible/incompressible.py`) had the same frozen-boundary
pattern, had never been pointed at `wallPressureExtrapolation`, and
`randomFlowIncompressible --bounded` detonated on it at *every* resolution for
that exact reason (Parts 42–45's "near-singular pure-Neumann operator"
framing was a real symptom but not the cause — the operator holds fine once
the wall stops lying to it). Generalise: **any composed pressure Jacobi in
this codebase that touches a wall needs a per-iterate wall closure, full
stop** — a frozen boundary value is not a simplification, it is a correctness
bug that happens to be survivable only when there is no real wall pressure
structure to get wrong (a quiescent column) or no wall at all (periodic).

### 1.19 The near-wall pressure closure is a family, ranked by how much of the wall's own state each commits to solving for (Part 56)

Every near-wall pressure closure this codebase has tried is an answer to the
same question — what value does a `kind==1` row contribute to the fluid's
pressure-gradient sum — differing only in how much is solved for versus
assumed:

1. **Frozen** (`_SHIFT_WALL_PRESSURE = None` / the historical default) — no
   dependence on the current iterate at all. Cheapest; does not contract
   (§1.18).
2. **Dirichlet extrapolation** (`'shepard'`, `'mls'`) — a value fit through
   the current fluid field, 0th or 1st order, recomputed every sweep. This is
   what actually fixes the wall-bounded cases measured so far.
3. **Neumann/reflection** (`'mirror'`, Adami et al. 2012, added Part 56) — a
   value anchored to the wall's own carried reading rather than returned
   outright (`p_b = 2·p_wall − shepard_f(p)`, the pressure analogue of
   `modules/mdbc/velocity.py`'s no-slip mirror). Holds alone but is measurably
   the weakest of the three (Part 57/58: fails cases the other two pass) and
   destabilises paired with a closed-domain gauge — not chased further, since
   nothing needs it once `'shepard'` works.
4. **Implicit unknown** (`band2018pb`) — no extrapolation at all; the wall row
   gets its own equation and diagonal, solved simultaneously with the fluid.
   Removes the near-wall rank deficiency at the operator level instead of
   patching the boundary condition. Costs a second class of unknowns and its
   own well-posedness bookkeeping (`bandWellPosedMask`).

That the two *middle* members are what actually fix `randomFlowIncompressible
--bounded`, not either end, is itself the lesson: full commitment
(`band2018pb`) works but is expensive and buys its own structural cost
(§1.20); no commitment (frozen) does not contract at all; a Robin-consistent
middle ground is enough.

### 1.20 Implicit boundary treatment buys incompressibility, not smoothness — a case can be bulk-correct and still fail on structure (Part 57/58)

`band2018pb`'s extended PPE removes the near-wall rank deficiency at its
source (§1.19 item 4), and on `randomFlowIncompressible --bounded`/`-obstacle`
it does measurably tighten density control versus `divergenceFree`'s
extrapolated closure (~4–5% peak overshoot vs ~7–8%, confirmed at both smoke
and `full` profile — not a resolution artifact). But on the *same* cases,
particle pairing and voids grow well past what `divergenceFree` shows there,
while kinetic energy still decays correctly throughout. The two failure modes
are independent: a scheme can get the bulk physics (density, momentum, energy
budget) exactly right while the particle *distribution* clumps or opens gaps
locally, and neither a KE trace nor a density band will catch it — only a
structural check (nearest-neighbour pairing fraction, void fraction) does.
Generalise: grading an incompressible scheme on density/KE alone is not
sufficient; a scheme that "passes" on those can still be visibly wrong in a
rendered video. `scripts/validate_scheme.py`'s `pairedFraction`/`voidFraction`
guards exist for exactly this reason — trust them alongside the density band,
not instead of it.
...
| **Promoting `columnCollapse`'s kernel `Wendland2` -> `Wendland4` for its post-impact pairing reduction** (Part 59/60) | Real at 400 steps (final `pairedFraction` 0.0987 -> 0.0633) but **does not hold at 3x the run length** -- the gap closes and slightly reverses by step 1200 (0.1743 vs 0.1909), i.e. a transient early-post-impact effect, not a persistent reduction. Also **does not transfer to `impact`**, the other pairing-prone case: there Wendland4's `pairedFraction` is higher throughout (final 0.2212 vs 0.1827), only `voidFraction` is lower -- the same void-down/pairing-up trade as `columnCollapse`'s tail, not a win. Not promoted anywhere; `dehnen2012`'s own claim (any Wendland kernel is pairing-stable at any `N_H`) is unaffected, since this never tested that claim in the first place (Part 59's own caveat). |
...
| `sample/regular.py` assigned particle mass from the nominal pre-snap grid spacing `dx**dim` while placing particles at the achieved, per-axis post-snap spacing `dn` (Part 54) | A uniform 0.5–1.4% mass/cell mismatch on every regularly-sampled case, worst at a wall (asymmetric support); traced as the seed of `randomFlowIncompressible --bounded`'s "unfixable" divergence (Part 48) before the real mechanism (§1.18) was found. Fixed at the source: mass and support now derive from the same `cellVolume = prod(dn_i)` the geometry actually uses |
| `c637785`'s rewrite of `solveIncompressible` dropped the per-iterate re-freeze of `kind != 0` pressure rows (`pressureB = torch.where(fluidMask, pressureB, boundaryPressure)`), leaving boundary/ghost pressure to drift through the same Jacobi update as fluid for the whole iteration (Part 56) | Restored while investigating the wall-pressure fix; tested in isolation and found not to change the case's divergence (the frozen-vs-extrapolated wall pressure dominates, §1.18) — correctness-restoring on its own, not the load-bearing fix |
---

## 8. Tooling

All in `scripts/`, all confirmed working when written, none required source
edits to use. Entries marked "removed" no longer exist on disk (pre-merge
cleanup, 09-04, DFSPH_IMPROVEMENT_PLAN.md "Pre-merge cleanup") — git history
has them if the exact script is ever needed again; what each one found is
recorded here and in §1/§9 regardless.
...
- **`scripts/splishsplash_compare/`** — *removed in the pre-merge cleanup pass
  (09-04), git history has it.* The SPlisHSPlasH cross-check kit (Parts
  35–38): `pysplishsplash` imported in both conda envs after Part 36's install
  fixes; a matched 2D `hydrostatic_2d.json` scene, an exact-fluid-state import
  driver (`import_and_run.py`, using committed `.npz` snapshots), the
  `n_h`/kernel sweep, the semi-periodic isolation variant, a video batch. What
  it found is durable (§1.12/§1.13, the setup matches for ~0.3s then warpSPH's
  composed Jacobi loses it); the kit itself needed an external
  `pysplishsplash` install and three committed binary snapshots, and had no
  ongoing role once Part 41 landed the actual fix.
- **`scripts/omnisph_compare/`** — *removed in the pre-merge cleanup pass
  (09-04), git history has it.* The omniSPH cross-check (Parts 40–41):
  `build_omnysph.sh` built `~/dev/omniSPH/omnySPH`'s `_core` against the warp
  env, exposing `SPHSimulation.timestep` + every substep; a matched
  hydrostatic column, an XSPH/BXSPH ablation, and `operator_diff.py`'s
  operator-by-operator diff against omniSPH's exact rest state. What it found
  is durable (§1.15, the operator-diff result that redirected the whole
  wall-pressure-closure track); the kit itself needed an external `~/dev/
  omniSPH` checkout outside this repo, so it could never run standalone here,
  and had no ongoing role once Part 41 landed the actual fix.
...
| 54 | 09-04 | **The lattice-density offset's `massRatio` half was never a sampler-fit fuzziness -- it is one line in `sample/regular.py`, now fixed at the source; `calibrateRestDensityMasses` (Part 53) is retired to `calibrateRestDensity`, which now only flips the kernel-side `calibrateNormalization` and checks a residual instead of scaling mass. A previously-unrelated, previously-unfixable `randomFlowIncompressible --bounded` divergence (DFSPH_IMPROVEMENT_PLAN.md Part 48, "nothing fixes it") turned out to share the same root cause and is now fixed too.** **Root cause, traced exactly (`sloshingTank` nx=100).** `buildPointCloud` picks one shared nominal spacing `dx` (the shorter axis's candidate), then snaps each axis independently to a whole number of cells: `nd = ceil(l/dx)`, `dn = l/nd`. Particles are placed at `dn`; mass was assigned from `dx**dim` (`regular.py:62`, pre-fix) -- the pre-snap, single global value, not the post-snap, per-axis one they actually landed on. Two distinct mechanisms stack: (1) **structural** -- any axis whose length is not an exact multiple of `dx` snaps by construction (measured 0.49% on the x-axis, `dn_x = l_x/183` against a `dx` sized for the y-axis); (2) **pure floating-point** -- even the *defining* axis is not safe: `l/dx` should recompute to exactly the intended integer (110.0), but float32 roundtrip landed it at `110.0000076` -- six orders of magnitude off -- and `ceil`, a hard step function sitting on that boundary, turned the residual into an entire extra row of cells (110 -> 111, measured 0.91%). Combined predicted offset `1.0049 * 1.0091 = 1.0140`, measured `massRatio = 1.014151` (Part 53's own number) -- matches to 4 digits. **The fix (`sample/regular.py`).** Compute `dn` per axis as before, but also accumulate `cellVolume = prod(dn_i)` and use THAT -- not `dx**dim` -- for both `masses` and `support` (`volumeToSupport`), so `h` and mass are mutually consistent with the geometry actually sampled, not a nominal target. The periodic half-cell offset and ghost-band width are switched from `dx` to each axis's own `dn` too (same bug, same fix, one abstraction level up); jitter width is now per-axis (`dn`) rather than a single global `dx`, for the same reason. **Result: `massRatio` is not reduced, it goes to 1 -- to float32 noise.** Measured 1e-7 to 4e-6 across `sloshingTank` nx in {30,60,100,150,200,333}, four orders of magnitude below the pre-fix 0.5-1.4%. Re-ran the four-way eval from Part 53 (raw / mass-only / kernel-only / both), 20 steps, nx = 60/100/200: **`raw` (no calibration of any kind) now matches every other config** -- step-0 `\|v\|max` 0.0096-0.0098 at every `nx` (was 0.198 / **1.703** / 0.0099 before this fix), step-20 `pairedFraction` 0.0000 at nx=100 (was 0.243). The only residual is `latticeFactor` (`L(n_h) ~ 1.0004`), exactly as designed -- confirms `LATTICE_DENSITY_PLAN.md`'s kernel-side correction now has the ONE job it was meant to have. **`calibrateRestDensityMasses` -> `calibrateRestDensity` (old name kept as an alias).** No longer scales mass at all in the expected path. Instead: (a) measures the same `latticeDensityDecomposition` split as Part 53, (b) if `massRatio` deviates from 1 by more than `tolerance` (default 1e-3, three orders above measured float32 noise, four below the old bug) on a plain regular unjittered sampling (`ctx.spec.samplingScheme == 'regular'` and no `jitter` param), **raises** by default (`onResidual='raise'|'warn'|'ignore'`) -- with the sampler fixed, a residual there means a corrupted IC (overlapping regions, an uneven SDF clip), not expected noise, and is no longer something to silently paper over; (c) flips `ctx.config.calibrateNormalization = True` if it is not already on, which is now the ONLY lever this function pulls -- literally "calibrates the C_d value" and nothing else. **Independent confirmation, found by accident: `tests/test_physics.py::test_randomFlowBoundedDoesNotDiverge`.** This xfail (DFSPH_IMPROVEMENT_PLAN.md Part 48: "OPEN REGRESSION -- diverges at every resolution... nothing fixes it", blamed on "the near-singular pure-Neumann near-wall CD operator") flipped to a reproducible XPASS the moment the sampler fix landed -- and this case **never calls `calibrateRestDensity`**, so the fix is the sampler change alone. Re-read against this finding, Part 48's own numbers fit: the density excursion it traced ("ρ spreads [0.98, 1.02] -> [0.2, 3.4]") starts from a small, uniform IC defect of exactly this bug's order of magnitude, worst at a wall where support is already asymmetric -- the CD operator did not manufacture the excursion, it failed to absorb one that was already seeded. Test promoted (xfail removed); DFSPH_IMPROVEMENT_PLAN.md's row updated, but **only nx=96 (the suite's fixture resolution) has been re-verified** -- the row's other resolutions (32/48/56/64/128) are not yet re-checked. |
| 55 | 09-04 | **Part 54's "FIXED" was itself wrong -- corrected. `randomFlowIncompressible --bounded` still diverges under `divergenceFree` at every resolution; the sampler fix only moved nx=96's onset just past the 20-step regression fixture, not past the failure.** *(Follow-up to Part 54's own flagged gap: only nx=96 had been re-verified, at the suite's 20 steps.)* Re-run past that window at nx in {32,48,56,64,96,128} (400-step budget, sized for t ~ 1.5-2 at each resolution's own adaptive `dt`): **all six diverge** (`kineticEnergy` -> non-finite). Onset times: nx=32 t~0.026, nx=48 t~0.186, nx=56 t~0.189, nx=64 t~0.190, nx=96 t~0.179 (just past the fixture's t~0.17 at 20 steps -- exactly why Part 54 read it as fixed), **nx=128 (this case's own shipped default) t~0.019, step 4** -- fastest of all six and matching Part 48's original pre-sampler-fix characterisation almost exactly, meaning the sampler bug was never the dominant driver at that resolution. `tests/test_physics.py::test_randomFlowBoundedDoesNotDiverge` re-xfailed (`strict=True`), fixture bumped to a dedicated 40-step run (past onset with margin) instead of the shared `STEPS=20`. **A `band2018pb`-style closed-domain gauge, ported into the VD+PS shift's `solveIncompressible` -- delays the divergence a lot, does not cure it. Landed opt-in, default off.** `modules/incompressible/incompressible.py` gained `_CLOSED_DOMAIN_GAUGE` (`'off'`/`'auto'`/`'always'`), mirroring `band2018pb.CLOSED_DOMAIN_GAUGE` (Part 50): when it fires, each Jacobi iterate is mean-centered over fluid rows instead of clamped to `p >= 0`, since a closed domain has no free surface for the clamp to be guarding against. The null-mode test (`rms|A.1| / rms|a_ii|`, boundary rows held at 0, one extra operator application per step) mirrors `band2018pb.bandConstantModeRatio` exactly, but **`'auto'` never fires on this operator**: measured 0.59-2.2 at nx=32 against the 0.25 threshold that fires reliably for `band2018pb`'s volume-centric operator -- this solve's plain `computePressureAccelIISPH` / `computePressureShiftIISPH` pair (BWJ23 Eq. 33 form) is genuinely less null at the wall, not merely mis-detected, consistent with Part 43's "near-singular, not singular" reading of the same family of operator under `omniIncompressible`. **`'always'` (forced) delays the divergence 20-140x without curing it:** nx=32 onset t~0.026 -> t~2.05 (gauge alone) -> **t~3.73** (gauge + `_CLOSED_BOX_SOURCE_PROJECT=True`, Part 42's mean-projected source, also still default off) -- `kineticEnergy` decays cleanly (0.418 -> 0.185, tracking the periodic variant's own decay) right up to a sudden single-step detonation both times, not a slow drift. nx=128 (the shipped default, the resolution that matters most): t~0.019 -> **t~0.39** with the same combination (~20x; smaller multiplier than nx=32 but still the largest lever found on this case since Part 42), KE flat/decaying (0.309 -> 0.301) up to a step-67 detonation. Verified inert (bit-close on vs off) on every other case that runs through this solve path: `tgv`, `kolmogorovIncompressible`, `staticBlob`, `randomFlowIncompressible` periodic. Also restored, while in this function: `pressureB = torch.where(fluidMask, pressureB, boundaryPressure)` after the per-iterate clamp -- the `c637785` rewrite had dropped the mid-loop re-freeze of `kind != 0` rows the function's own docstring still documents (rows 183-199 of the pre-Part-55 file), so boundary/ghost pressure was drifting via the same Jacobi update as fluid for the whole iteration instead of staying pinned at its incoming value; tested in isolation and found not to change the divergence (the operator's own near-singularity dominates), but correctness-restoring and covered by the same inertness checks above. Full `test_physics.py` + `test_runner.py` suite: 91 passed / 1 xfail, unchanged. No `@wp.kernel` touched. **What's left (DFSPH_IMPROVEMENT_PLAN.md Active track item 2a):** find what turns the delay into a hold -- `band2018pb` itself needed a Tikhonov diagonal floor and a well-posed-row mask on top of its own gauge (Part 50) before it actually held; this operator may need the same, or may not be able to get there at all, in which case item 2b (re-point the case's default to `band2018pb`) is the real answer. |
| 56 | 09-04 | **`randomFlowIncompressible --bounded` is FIXED for real under `divergenceFree` -- the actual mechanism was the wall pressure freezing mid-iteration, not the near-singular operator Part 55's gauge tried to patch. `_SHIFT_WALL_PRESSURE = 'shepard'` (previously-scaffolded, previously-untested, now the default) holds every resolution cleanly. Also: a new Adami-mirror pressure mode, and the family framing that motivated checking it.** *(User's steer: "check different pressure projections too, e.g. mls pressure projection and pressure mirroring too. pbsph should be a specific pressure projection in a way as well so this makes a family of terms".)* **Root cause.** `modules/incompressible/incompressible.py`'s `solveIncompressible` (the VD+PS shift's constant-density Jacobi) captures `boundaryPressure = particles.pressures.clone()` once before the loop and re-imposes it on `kind != 0` rows every iterate (`pressureB = torch.where(fluidMask, pressureB, boundaryPressure)`) -- the wall pressure never depends on the current fluid iterate at all. This is exactly the non-contraction Part 41 diagnosed for `omniIncompressible`'s own composed Jacobi: "the near-wall iteration matrix `D` carries a wall term the operator `A p` does not." Part 41's fix (`wallPressureExtrapolation` -- recompute `p_b` from the current fluid pressure every sweep, no relaxation, no carried state) was ported into *this* solve already, as the `_SHIFT_WALL_PRESSURE` scaffold (`None`/`'shepard'`/`'mls'`) -- but left at `None` and never actually exercised on the case it would have fixed. **Fix: `_SHIFT_WALL_PRESSURE` defaults to `'shepard'`.** Swept `{None, 'shepard', 'mls', 'mirror'} x {_CLOSED_DOMAIN_GAUGE 'off'/'always'}` at nx=32 first (cheap): `none/off` detonates at t~0.025 as expected; **`shepard/off`, `mls/off`, and `mirror/off` (see below) all hold the full 80-step probe (t~1.8) with clean KE decay -- no gauge needed at all.** Extended to confirman real hold, not another delay: `shepard`/`mls`/`mirror` at nx=32 ran the full 400-step budget (t=8.7-11.5) with **no divergence**, KE monotonically decaying to 0.06-0.14 from 0.418. Then the full resolution ladder this case's history is built on, all `shepard/off`, 300-400 steps each, **all clean**: nx=48 (KE 0.377->0.281, t=3.07), nx=56 (0.349->0.270, t=2.87), nx=64 (0.345->0.286, t=2.47), nx=96 (0.322->0.278, t=1.81), **nx=128 -- this case's own shipped default, the fastest-diverging resolution in every prior characterisation -- 0.309->0.274, t=1.46, 300 steps, no divergence.** `--obstacle` (internal solid obstacle, same code path) also holds at nx=64/150 steps (KE 0.322->0.282). **New: `'mirror'` mode in `modules/incompressible/wallPressure.py`.** Adami et al. 2012's wall-pressure BC, built to match the *reflection* form `modules/mdbc/velocity.py`'s no-slip velocity mirror already uses (`u_g = 2*bodyVelocity - qVel`): `p_b = 2*p_wall - shepard_f(p)`, reflecting the Shepard-interpolated fluid pressure about the wall's own currently-held value (`state.pressures` at the boundary rows) rather than returning the fluid value directly -- a Neumann-style closure (holds the two-point finite-difference `dp/dn` fixed at whatever `p_wall` encodes) where `'shepard'` is Dirichlet-style (returns the fluid value outright). The Adami body-force correction term (`rho_f * (g - a_wall) . (r_wall - r_f)`) is **not implemented** -- it needs a vector *moment* of the neighbourhood, which `Interpolate` cannot return -- and raises if a non-`None` `bodyForce` is passed rather than silently ignoring it; every current caller of this solve runs with the body force excluded anyway (`_psShift` only fires when `not gravityConfig.active`), so the omission is provably a no-op for every case that reaches this solve today. `'mirror'` holds the case alone at every resolution measured (nx=32 KE 0.418->0.139 t=8.66; nx=128 KE 0.309->0.321 t=1.21 -- the only config where KE *rose* rather than decayed, still bounded) but is the weakest of the three, and **`mirror + _CLOSED_DOMAIN_GAUGE='always'` diverges** (nx=32, t~0.47) where `shepard + gauge` and `mls + gauge` both hold and decay *faster* than gauge-off (nx=32 KE ->0.266/0.274 vs 0.101/0.060) -- an interaction not root-caused, since neither the gauge nor `'mirror'` is needed once `'shepard'` alone works. **The pressure-projection family, made explicit (the user's ask).** Ranked by how much of the wall row's own state the closure commits to solving for rather than assuming: **frozen** (`None` -- no dependence on the current iterate, the historical default, does not contract) -> **Dirichlet extrapolation** (`'shepard'`/`'mls'` -- a value fit through the current fluid field, 0th/1st order) -> **Neumann/reflection** (`'mirror'` -- a value anchored to the wall's own reading rather than returned outright) -> **implicit unknown** (`band2018pb` -- no extrapolation at all, the wall row gets its own equation and diagonal, Eq. 10/20, solved simultaneously with the fluid; the near-wall rank deficiency Parts 42-45 characterised at length is removed at the operator level instead of patched at the boundary). That the two *middle* members fix this case, not the frozen end (too little commitment, does not contract) or the implicit end (`band2018pb` also holds it, Part 50, but at the cost of a second unknown class and its own well-posedness bookkeeping), is itself the finding: a Robin-consistent per-iterate closure is enough here, and cheaper than going fully implicit. **Landed.** `_SHIFT_WALL_PRESSURE = 'shepard'` is the new default (was `None`); blast radius is provably narrow -- the only case in the codebase with both `kind==1` rows and `_psShift` active (non-gravity) is `randomFlowIncompressible --bounded`/`--obstacle`, every other caller of this solve (`tgv`/`shearWave`/`staticBlob`'s `relaxLattice`, `kolmogorovIncompressible`, the periodic random flow) has no boundary particles so the new default is a no-op there, verified bit-close on/off at 20 steps each. `tests/test_physics.py::test_randomFlowBoundedDoesNotDiverge` un-xfailed and strengthened (`kinetic[-1] < 1.1*kinetic[0]`, density band [0.85, 1.15], matching the periodic variant's assertion style). Full suite: 92 passed / 0 xfail (was 91/1). `gradcheck_incompressible` green -- no `@wp.kernel` touched, `_adamiMirror`/`_shepardValue` are composed from the existing `warpOperation`/`Interpolate` primitive. **What's left:** the `'mirror'`+gauge interaction is unexplained; `_SHIFT_WALL_PRESSURE`'s new default has no effect on `hydrostaticColumn`/`dambreak` (they route through the in-step CD fold under gravity, not this `finalize` shift, so `_psShift` never fires for them) but the mirror mode's body-force term would need implementing before it could ever help a gravity-driven case that *does* reach this solve; whether `'mls'`'s slightly better decay is worth its extra cost is untested at longer horizons. |
| 57 | 09-04 | **`divergenceFree` pushed through `scripts/validate_scheme.py`'s full case table, and the pressure-closure family (`shepard`/`mls`/`mirror`/`band2018pb`) graded head to head across every boundary case, not just `randomFlowIncompressible --bounded`.** *(User's steer: "run the divergence free solver through its paces, cases with no boundary can only be run once, for boundary cases run them with mls, mirror and band and compare their behavior. use the validation sweep... add a --fluidOnly and --boundaryOnly flag".)* **Harness additions.** `Experiment.hasBoundary: bool` on all 14 experiments (7 fluid-only: `tgv`/`kolmogorov`/`shearWave`/`staticBlob`/`randomFlow-periodic`/`impact`/`squarePatch`; 7 boundary: `randomFlow-bounded`/`-obstacle`/`hydrostaticColumn-64`/`-128`/`dambreak`/`columnCollapse`/`sloshingTank`, confirmed by sampling `kind==1` counts directly, not assumed). `--fluidOnly`/`--boundaryOnly` (mutually exclusive) filter on it. `--wallPressure {shepard,mls,mirror,none}` sets `WARPSPH_WALL_PRESSURE_MODE` in the parent's `os.environ` once, which every child subprocess inherits (`subprocess.run`'s default with no `env=` override) -- reaching **both** `modules/incompressible/incompressible.py`'s `_SHIFT_WALL_PRESSURE` (the VD+PS shift, non-gravity boundary cases) **and** `schemes/omniIncompressible.py`'s `WALL_PRESSURE_MODE` (the in-step CD fold `dfsph_step` uses under `INSTEP_CD`, gravity-active boundary cases) from one flag, since `divergenceFree` routes a boundary case through whichever of the two the case's own gravity setting selects. Both modules gained an `os.environ.get('WARPSPH_WALL_PRESSURE_MODE')` read at import time (`'none'` -> `None`), falling back to their compiled-in `'shepard'` default when unset -- verified inert (default behaviour bit-identical) when the variable is absent. **Results, smoke profile (`--scheme divergenceFree`, then `--scheme band2018pb` for the same 7 boundary experiments).** Fluid-only: 6/7 PASS, `impact` FAILs on pairing (0.231) -- pre-existing (`impact` has zero `kind==1` particles, confirmed by the same `hasBoundary` audit, so no wall-pressure closure of any kind can be the cause; unrelated to this session). Boundary, by failed check (not just PASS/FAIL, since the label alone conflates unrelated failure modes): `randomFlow-bounded`/`-obstacle` -- `shepard`/`mls` fail ONLY the harness's strict `max density <= 1.05` (measured 1.07-1.08, the same non-catastrophic overshoot Part 56 already characterised as a genuine hold, not a divergence -- the harness's `RANDOMFLOW_RHO=(0.95,1.05)` band is simply tighter than `test_physics.py`'s `[0.85, 1.15]`); `mirror` fails the same density check **plus** pairing-appeared and min-density -- strictly worse, corroborating Part 56's single-case finding across a second, independent case; `band2018pb` is the only one to **pass** the density check (its design intent -- tighter incompressibility) but fails on pairing growth (0.07->0.17-0.18, >2x) and voids -- a genuinely different failure mode (structural roughness, not compressibility). `hydrostaticColumn-128`/`dambreak`: `shepard` and `mls` both **PASS cleanly**; `mirror` **uniquely fails both** (pairing growth on the column; pairing + voids on dambreak) -- two more independent confirmations that `mirror` is the weakest of the three, not just weaker on the one case Part 56 measured; `band2018pb` passes the column but fails dambreak on voids + `densityP05`, which is `band2018pb`'s own already-documented free-surface gap (Part 51: "fixes walls and inherits the free-surface problem"), not a wall-pressure-closure artifact. `hydrostaticColumn-64` (smoke nx=32), `columnCollapse`, `sloshingTank`: **all four configurations fail identically** -- a smoke-profile resolution/step-count ceiling (100-150 steps at coarse `nx`, well short of the validated `full`-profile budgets in Part 50/51/53), not a wall-pressure-closure effect; `sloshingTank`'s Sensor-1 peak (9-25 Pa against a 1-30 kPa check) confirms the smoke profile is simply too short for the sloshing wave to build real pressure, independent of scheme or mode. **Net read:** `shepard` (the landed default) and `mls` are behaviourally equivalent across the whole boundary-case table at this profile -- neither is a clear win over the other, matching Part 56's narrower finding; `mirror` should not be promoted anywhere, it is measurably worse on every case that discriminates; `band2018pb` is a genuine trade-off (tighter density, rougher particle distribution) rather than a strict improvement or regression, worth a `full`-profile look before concluding further (smoke's coarse `nx`/short steps are exactly what the harness's own docstring warns is a coverage check, not a release grade). One fixed driver-script lesson recorded for next time: `validate_scheme.py` intentionally exits 1 on a graded `FAIL` (not a crash), so a wrapper script chaining several sweeps must not use `set -e` -- it silently truncated this session's first attempt after the very first sub-sweep. |
| 58 | 09-04 | **Part 57's two open questions settled at `--profile full --video`: the resolution-artifact hypothesis was right for 2 of 3 suspect cases and wrong for one; `band2018pb`'s pairing/void regression on `randomFlow` is confirmed real, not a coarse-grid artifact.** *(User's follow-up: "run them with divergence free with higher resolution but also with video output... also include the random flow for divergence free so theres a better understanding of what the band violation means.")* Two targeted `full`+`--video` runs (not a blanket rerun of all 4 configs -- `mirror` is already disqualified and not worth the compute): `--scheme divergenceFree --cases hydrostaticColumn-64 columnCollapse sloshingTank randomFlow-bounded randomFlow-obstacle` and `--scheme band2018pb --cases randomFlow-bounded randomFlow-obstacle dambreak`. **`hydrostaticColumn-64` and `sloshingTank` both flip to PASS** (nx=64/400 steps; nx=200/~14900 steps, 7s physics, ~19 min) -- confirms the smoke failures were exactly what they looked like, the smoke profile's 100/150 steps at coarse `nx` simply too short for the gradient / wave to build, not a real problem. **`columnCollapse` does NOT flip -- it fails at full resolution too, but on a different, previously-undiagnosed mechanism:** `nPenetrating` stays exactly 0 the whole run (the wall holds, the case's own dedicated regression test would still pass), but `pairedFraction` grows 0.3% -> 9.0% after the collapse impact -- real post-impact clumping in the splash, not a wall or divergence issue, and not something the smoke run (which also failed, just on a smaller number) was wrong about. **`randomFlow-bounded`/`-obstacle` under `divergenceFree`** confirm the density-band overshoot persists at nx=64/200 steps (max 1.084 / 1.070, `min density` also dips fractionally under 0.95 on the bounded variant) -- same magnitude as smoke, not shrinking with resolution, consistent with Part 56/57's read that this is a persistent property of the `shepard` closure on this case rather than a transient. **`band2018pb`'s pairing/void failures on `randomFlow-bounded`/`-obstacle` are CONFIRMED real, not smoke-resolution noise:** at nx=64/200 steps, density passes cleanly this time (max 1.044 / 1.046, vs `divergenceFree`'s 1.084/1.070 -- `band2018pb` genuinely is more incompressible here, exactly its design intent), but `pairedFraction` grows 5%->15% / 5%->13% and `voidFraction` 0%->9% / 0%->6% -- both well past the harness's growth guard, at the case's own validated resolution, with KE still decaying correctly throughout (0.446->0.213 / 0.421->0.159). So `band2018pb` has a genuine, resolution-independent trade-off on this case family: tighter density control, rougher particle distribution. **`band2018pb` on `dambreak`** (nx=64, 600 steps) reproduces Part 51's already-documented free-surface gap exactly: `nPenetrating` stays 0 (wall holds), but `voidFraction` grows 8%->19% and `densityP05` drops to ~0.25-0.29 -- not new information, just full-resolution confirmation that Part 51's finding ("fixes walls and inherits the free-surface problem," no free-surface treatment in the paper's method) holds at this scale too. Videos sent to the user for all 8 experiments rather than described from band values alone. **Net effect on the Active track:** item 2b (`band2018pb` as the wall-bounded scheme) is now better characterised, not resolved -- it is a real trade (density vs. structure), and `columnCollapse`'s post-impact pairing under `divergenceFree` is a newly-surfaced, previously unflagged issue worth its own investigation (separate from the wall-pressure-closure work; `nPenetrating=0` throughout means it is not urgent). |
| 59 | 09-04 | **`columnCollapse`'s post-impact pairing (Part 58) is real and substantially reduced by switching the kernel from Wendland2 to Wendland4, at otherwise-identical settings.** *(User's follow-up, with the Dehnen & Aly 2012 kernel-pairing paper supplied and added to `literature/` -- see below.)* Same experiment as Part 58 (`--scheme divergenceFree`, nx=64, 400 steps, `cflFactor 0.2`), `kernel` swapped `'Wendland2'` -> `'Wendland4'`, `n_h = 4` (`targetNeighbors`) unchanged on both. **Wendland4 holds up cleanly** (`diverged=False`, `nPenetrating` stays 0, KE trajectory near-identical to Wendland2: 0.3276 vs 0.3267 final) **and pairing is substantially lower, growing from a real trend, not two noisy sample points:** the two runs track together (`pairedFraction` both ~0 through the pre-impact phase) until the collapse impact at t~0.2, where they diverge and the gap widens monotonically for the rest of the run -- final `pairedFraction` **0.0987 (Wendland2) vs 0.0633 (Wendland4)**, peak **0.0987 vs 0.0757**, roughly a 25-35% reduction depending on where in the tail it's read. `voidFraction` is unaffected (both peak 0.0164) and the density band is, if anything, slightly better under Wendland4 (`rho` [0.507, 1.013] vs [0.471, 1.008] -- the free-surface floor is higher, not lower). Wall-clock cost was a wash at this `n_h` (9.2s vs 10.0s for 400 steps -- Wendland4's larger natural support at the *same* `n_h` does not cost more neighbours here, since `n_h` is the thing held fixed, not the kernel's own compact-support radius in units of `dx`). **Caveat, worth being precise about:** this is not quite testing Dehnen & Aly's own headline claim -- their result is that *any* Wendland kernel avoids the classical pairing instability at *any* `N_H`, not that higher-order Wendland kernels are progressively more pairing-resistant than lower-order ones at matched `N_H`; the mechanism behind the Wendland2-vs-Wendland4 gap measured here (both nominally "pairing-stable" per the paper) is more likely the smoother density/force estimate a higher-order kernel gives at the same neighbour count than a difference in fundamental stability -- worth reading the paper's own §3/§4 before making a stronger claim than "measurably less pairing here." **Literature: `dehnen2012` promoted from the extended (bibliographic-only) set to the curated core** (`literature/ABSTRACTS.md`, new "Kernel choice and the pairing instability" section) with a codebase-specific relevance note, since it is now load-bearing for reasoning about this result and not just background -- `references.bib` and `MANIFEST.md` updated to match (core 38->39, extended 78->77), `ABSTRACTS_EXTENDED.md`'s now-duplicate entry removed, `scripts/check_literature.py` green (`dehnen2012` verifies verbatim against the PDF's own text layer, arXiv copy `arXiv:1204.2471v2`). **Not yet done:** a longer run / higher resolution to see whether the gap keeps widening or plateaus; the same A/B on `impact` (the other case with a pre-existing, unrelated pairing issue) to see if it transfers; and whether Wendland4 is worth promoting to a default anywhere, which needs more than one case's worth of evidence and is the user's call, not made here. |
| 60 | 09-19 | **Both of Part 59's open questions answered, and the answer is: don't promote Wendland4.** Same `columnCollapse` config (`--scheme divergenceFree`, nx=64, `cflFactor 0.2`, `n_h=4`, `semiImplicitEuler`, `SuperSymmetric`), run 3x longer (1200 steps, not 400): **the gap does not widen, it closes and slightly reverses.** Thirds-of-run `pairedFraction` means track each other closely throughout -- Wendland2 0.0403 -> 0.1556 -> 0.1857, Wendland4 0.0321 -> 0.1389 -> 0.1825 -- and by the final step Wendland4 is marginally *worse*, not better (final 0.1909 vs Wendland2's 0.1743; peak 0.2130 vs 0.2196, now roughly tied rather than a 25-35% gap). Part 59's win was real but transient: an early-post-impact effect that fades as both kernels converge toward the same long-run clumping level. `voidFraction` is the one place a real, persistent gap survives (final 0.1021 vs 0.1431, Wendland4 lower throughout) -- but that is not what item 2 was chasing. Also newly visible only at this horizon: Wendland2 shows `nPenetrating_max=1` (one particle briefly crosses the wall band) where the Part 58/59 characterisation, run only to 400 steps, called wall integrity "exactly correct"; Wendland4 still holds `nPenetrating=0` at 1200 steps. Not promoted on this alone -- one crossing is far short of a wall failure, and the case already has `mdbcNoPenetrationShift` for exactly this axis -- but the "wall holds exactly" claim was scoped to 400 steps, not indefinitely, and should be stated that way going forward. **The transfer test (`impact`, same kernel A/B, `--scheme divergenceFree`, nx=64, 400 steps, kernel forced explicitly on both legs since the harness's `INCOMPRESSIBLE_PRESET` would otherwise silently pick Wendland2 while `impact`'s own un-overridden default is Wendland4) does not confirm either -- if anything it reverses:** Wendland4's `pairedFraction` is higher than Wendland2's throughout (thirds 0.0598/0.1342/0.2117 vs 0.0688/0.1387/0.1933; final 0.2212 vs 0.1827; peak 0.2981 vs 0.2788), while `voidFraction` is lower (final 0.2716 vs 0.3269) -- the same void-down/pairing-up trade seen on `columnCollapse`'s tail, not a clean win on either case. **Verdict: item 2 is closed, decided against promoting.** The single-case, single-run-length result that motivated it does not generalise across run length on its own case or transfer to the other pairing-prone case; `DFSPH_IMPROVEMENT_PLAN.md`'s open item 2 and `DFSPH_TODO.md`'s item 1 are closed on this basis, no code changed (`kernel='Wendland2'` stays `columnCollapse`'s default). `dehnen2012` stays in the curated literature set regardless -- its own claim (any Wendland kernel is pairing-*stable* at any `N_H`) was never what this measured, and remains correctly scoped in Part 59's own caveat. |
| 61 | 09-19 | **`relaxationFactor=0.3`'s TGV-family ~15% margin does not hold on a bounded wall subproblem (down to ~7% at nx=128) -- confirmed by `--mode spectrum`. Found and fixed a real bug along the way (the constant-density solve silently ignored this field entirely). Lowering the default to spend the margin back was tried and NOT adopted: a real A/B, not the "free" result a first, broken A/B seemed to show.** *(DFSPH_TODO.md item 1, "roughly cheapest/most-concrete.")* **The margin measurement.** `scripts/probe_boundaryOperatorTerms.py --mode spectrum` (`randomFlowIncompressible --bounded`, the only case whose `captureState()` hook reaches `solveIncompressible` -- gravity-active cases route through a different in-step CD fold this probe does not touch) at nx in {32,64,96,128}: the shipped `staticBoundary` operator's `rho(D^-1 A)` is 5.82/5.78/5.93/6.19, giving `omega=0.3`'s margin against `2/rho` as 12.7%/13.3%/11.1%/**7.1%** -- shrinking with resolution, well short of the TGV family's ~15% (`rho~5.64`, from the un-walled operator) the docstring had been quoting. The disqualified alternative operator (`boundaryOperatorTerms='full'`, not shipped but selectable) is **already outside the window at every resolution measured** (`omega/window` 1.02-1.05) -- a previously-uncharacterised latent trap: selecting it on this solver family without also changing `relaxationFactor`/`relaxationMode` is provably non-contracting from the spectral radius alone, independent of the wall-treatment physics question `BoundaryOperatorTerms` is actually about. **A bug, found while building the A/B to test lowering `omega`.** First attempt monkeypatched `warpSPH.configurations.incompressible.buildDefaultPSConfig`/`buildDefaultDFConfig` (the pattern several existing probes use) and got byte-identical results at every "different" `omega` tried -- traced to `IncompressibleSolverConfig`'s `pressureSolver`/`divergenceFreeSolver` fields using `field(default_factory=buildDefaultPSConfig)`, which dataclasses binds to the function *object* at `solver.py`'s own import time, immune to reassigning that name anywhere else; confirmed by instrumenting the real call site directly (a patch attempt of `0.123456` still reached `solveIncompressible` as whatever `solver.py`'s own default was). Redone correctly by mutating the live `schemeConfig.solverConfig` object in place from inside a `solveIncompressible` capture hook (the object is built once at setup and reused by reference for the whole run, so this sticks). **That correct A/B surfaced a second, independent, real bug:** `incompressible.py`'s constant-density Jacobi loop hardcoded `omega = 0.3` inside the iteration (shadowing the `omega` already correctly read from `schemeConfig.solverConfig.pressureSolver.relaxationFactor` a few lines above, every single iteration) -- exactly the "Known-open" item `DFSPH_IMPROVEMENT_PLAN.md`'s pre-merge cleanup log had already flagged and deliberately left alone pending its own validation. `divergenceFree.py`'s analogous DF loop never had this shadow. **Fixed**: the hardcode line is removed, `omega` now stays the configured value. Validated behaviour-preserving at the unchanged default (0.3): reproduced the pre-fix numbers exactly by forcing the config to 0.3 on both the old (git-stashed) and new code, and the full `test_physics.py`/`test_runner.py`/`test_incompressibleKrylov.py` suite is unchanged (one pre-existing, unrelated failure both before and after -- `test_everyCaseDeclaresItsParamsAsScalarsOrLists` on `dambreak.surfacePressureProbes` being a tuple, confirmed via the same stash to predate this session entirely; not this track's concern). No `@wp.kernel` touched, no gradcheck needed. **With the bug fixed, the real `omega=0.3` vs `0.27` A/B (bounded case, nx=32/128, 50/200 steps):** density bounds tighten slightly at 0.27 (nx=128 `maxDensity` 1.0802->1.0640, `minDensity` 0.9813->0.9833) -- but peak `maxVelocity` gets *worse*, growing with resolution: +6% at nx=32 (1.394->1.466), **+21% at nx=128** (1.616->1.961). `kineticEnergy` is unaffected either way (<0.4%). On periodic `tgv` (nx=64), both values are within noise of each other, as expected for a well-conditioned operator with a large margin already. **Verdict: not adopted.** This is a genuine trade (tighter bulk density for a noisier velocity peak that gets worse exactly where the margin argument was strongest), not the "flat inside the window, no cost" result the first (broken) A/B seemed to show -- `relaxationFactor` stays `0.3` on both solvers. The bug fix ships regardless, since it is a correctness fix independent of which default value is chosen: the field can now actually change the constant-density solve's behaviour, where before it silently could not. **Left open:** the gravity-active in-step CD fold's own margin is unmeasured (different code path, no probe hook); whether `omega=0.27`'s density-vs-velocity trade is worth it on a case that's graded on density (not `maxVelocity`) is a separate, un-asked question this session did not chase further. |
| 62 | 09-19 | **`band2018pb`'s smoke-profile `columnCollapse` FAIL is not a resolution artifact by the harness's own automated grade, but the mechanism turns out to already be a known, accepted one -- and `sloshingTank` fails too, on a different, also-already-known mechanism.** *(DFSPH_TODO.md item 2, "one run settles it.")* `scripts/validate_scheme.py --scheme band2018pb --cases hydrostaticColumn-64 columnCollapse sloshingTank --profile full --video`. **`hydrostaticColumn-64` flips to PASS** (nx=64/400 steps, `pressureSlopeRatio` 1.002, quiescent), same resolution-artifact story Part 58 found for `divergenceFree` on this case. **`columnCollapse` does NOT flip** (nx=64/400 steps): fails the harness's automated `pairing not growing (<2x)` guard, `pairedFraction` 0.026 -> 0.084 (3.2x), `nPenetrating` stays 0 throughout. But the *absolute* magnitude (tail 0.084, peak 0.092) now lands in the same range as `divergenceFree`'s own post-impact clumping on this exact case (Part 58/59: final 0.090-0.099, also technically fails the same automated guard and is accepted as cosmetic, not structural) -- not the outsized regression the smoke-profile comparison suggested (0.138 vs `divergenceFree`'s smoke-scale 0.033-0.066). At full resolution the two schemes converge to comparable pairing levels; the smoke-profile gap was the artifact, even though the FAIL grade itself persists (for the same already-accepted reason `divergenceFree`'s does). **`sloshingTank` also does NOT flip to PASS, but on a different check than `columnCollapse`'s:** full 7s run (nx=200, 15726 steps, 676s), Sensor-1 pressure checks both pass (peak 1.12e4 Pa, in the 1-30 kPa band; not stuck at zero) and pairing holds (0.070 -> 0.098, under the 2x guard) -- but `voidFraction` grows 0.29% -> 2.96% (tail median), peaking 4.36%, over the harness's 2% cap. This is a new *instance* of the free-surface void gap already diagnosed on `dambreak`/`randomFlow` (Part 51/58: `band2018pb`'s extended-PPE method has no free-surface treatment), not a new mechanism -- `sloshingTank`'s violent slosh wave gives it the same kind of free surface those cases have. **Net: item 2 is closed.** Neither case flips to PASS, so the smoke result was not purely a too-short-run artifact -- but neither is it new information: `columnCollapse`'s failure converges to `divergenceFree`'s own already-accepted pairing level, and `sloshingTank`'s failure is the already-documented void gap showing up on a third case. `DFSPH_IMPROVEMENT_PLAN.md`'s "graded across the same table" note for `band2018pb` extended to cover all three cases; `hydrostaticColumn-64` added to the PASS side, `columnCollapse`/`sloshingTank` added to the known-trade side alongside `randomFlow`/`dambreak`. Videos in `sweeps/validate-band2018pb-full-20260919-125858/`. |
| 63 | 09-19 | **The `DensityEvolution.hybrid` mDBC hypothesis (DFSPH_TODO.md item 1) can't be tested against current code -- `DensityEvolution` is dead, not live, on the production `divergenceFree`/`omniIncompressible` path.** *(Checked before running the "cheap test" the TODO item proposed -- re-summing for the extrapolation only -- since it changes a branch that turned out not to exist any more.)* Direct code reading: both places `divergenceFree.py`/`systems/incompressible.py` used to branch on `resolveDensityEvolution(...)` are now commented-out dead code (`# _densityEvolution = resolveDensityEvolution(...)` / `# if _densityEvolution is DensityEvolution.summation: ...`), each followed by an unconditional line doing the `summation`-mode thing regardless of the config (`currentState.densities = computeDensities(...)` at the top of `divergenceFree_step`; `self.state.densities.copy_(lastState.densities)` in `finalize`). `grep -rl densityEvolution src/warpSPH` confirms no other scheme (`band2018pb.py`, `omniIncompressible.py`, `iisph`, `dfsphReference.py`) reads the field either -- it is config-dead across the whole incompressible track, not just this one call site. Confirmed empirically too: `scripts/probe_densityEvolution.py --nx 32 --nsteps 60`, all three `densityEvolution` settings (`summation`/`continuity`/`hybrid`) produce identical `t_final` (1.3332) and near-identical wall time (34.9/34.1/34.0s) -- consistent with running the same code regardless of the setting (the probe's own drift instrumentation reports `nan` throughout, a separate monkeypatch-technique issue like Part 61's `buildDefaultPSConfig` one, not relied on for this conclusion). `git log -S` on the removed branch traces it to commit `71a8ae7` (09-02, the same large "tmp commit" that introduced `band2018pb`/the boundary-operator reformulation) -- i.e. the mechanism went dead **before** Part 10's own measurement was even written up, and has stayed dead through the Part 46-58 rewrite the other two "re-scope before running" TODO items already flag. **Verdict: item 1 is closed as moot, not investigated.** The `hybrid`-dies-at-286-steps behaviour Part 10 measured was real *for the code that existed then*; testing today's `computeMdbcDensity` call against it tests nothing, since no code path reads `densityEvolution` any more. If `DensityEvolution` is wanted again, it needs re-wiring into the current step function first (a design decision -- which call site, whether `hybrid`'s "fresh summation for the CD/shift solve only" semantics still make sense post-rewrite -- not a bug fix), not a mDBC tweak. `DensityEvolution`/`resolveDensityEvolution` stay in `solver.py` as-is (still exported, still a valid config field, just currently inert) -- not removed here, since deciding whether to revive or delete the mechanism is a separate call this session didn't make. |
| 64 | 09-19 | **`scripts/probe_dambreakEnergyBudget.py` (the "net -8.5, 85% of the loss" figures DFSPH_TODO.md's dam-break item is built on) is broken against current code -- it targets a code path that is no longer the default.** *(Checked before launching the multi-`nx` convergence sweep that item asked for, since the probe hadn't been run since Part 19/21, 08-29.)* `python scripts/probe_dambreakEnergyBudget.py --nx 32 --tLimit 0.3` raises `KeyError: 'a_DF'` inside `patched_finalize`: `captured['a_DF']` is written by a wrapper the probe installs around `dfsph_mod.solveDivergenceFree` (`schemes/divergenceFree.py`'s `solveDivergenceFree`, re-exported from `modules/incompressible/divergenceFree.py`), but that function is only reached when the module-level `DIVERGENCE_SOLVER` flag in `schemes/divergenceFree.py` is `'vdps'` -- the current default is `DIVERGENCE_SOLVER = 'omni'` (with `SOLVE_ORDER = 'div_then_cd'`, `INSTEP_CD = 'auto'`), which routes through `_omniPass`/`omniIncompressible._solve` instead and never calls `solveDivergenceFree` at all. So the probe's wrapper never fires, `captured['a_DF']` never gets written, and `patched_finalize` (which unconditionally expects it) crashes. **This is the same class of staleness as Part 63's `DensityEvolution` finding, on the other big open TODO item** -- the probe (and by extension the "net -8.5, 85% of the loss" numbers the plan quotes) dates to Part 19 (08-29), before the `'omni'` solver became the shipped default. The dam-break dissipation question itself is still open and still worth answering, but **not with this probe as-is**: it needs its capture points moved to `_omniPass`'s own accel/pressure terms (or run once against the non-default `DIVERGENCE_SOLVER='vdps'` path to see whether the mechanism transfers, which is itself an open question, not an assumption) before an `nx` sweep is worth the compute. Not fixed here -- re-scoping which capture points map to the `'omni'` path's actual force decomposition is a small design question, not a one-line fix, and doing it carelessly would produce numbers that look like Part 19's but describe a different solver. `DFSPH_IMPROVEMENT_PLAN.md` item 3 and `DFSPH_TODO.md`'s "Worth picking up" list updated to flag this rather than presenting the item as directly runnable. |
