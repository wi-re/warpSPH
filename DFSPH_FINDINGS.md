...
| **`omniIncompressible` as a general incompressible scheme** (Part 42) | Holds the *periodic* cases (`tgv` KE ratio 0.79 / bounded — over-dissipative from the `XSPH_FLUID = 0.05` default; `kolmogorovIncompressible` KE → 1.38) — its 3-iter divergence pass keeps it bounded where `iisph` blows up. But **diverges on `randomFlowIncompressible --bounded`** (KE → ~1e31, `|v|max` → 1e15, within a few steps) *with* `WALL_PRESSURE_MODE = 'mls'` active. So Part 41's MLS wall-pressure closure holds the quiescent `hydrostaticColumn` but not a wall-bounded shear flow — `OMEGA = 0.3` / 3-iter-divergence / Akinci band is not robust there. dambreak nx=64 still preserved; a full dambreak grade untried. |
---
